...
test_non_leak = test_data %>% subset(!(user_id %in% output_leak_1$id))

# Using a smaller dataset to see if that gets us anywhere
...
test_non_leak = test_data %>% subset(!(user_id %in% c(output_leak_1$id, output_leak_2$id)))

test_non_leak$hotel_cluster = best_five
...
write.csv(submission, "../Data/kaggle/expedia/submissions/sub4.csv", row.names = F)
